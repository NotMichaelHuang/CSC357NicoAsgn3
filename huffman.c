 ...
 * */
short int *link_to_array(DataNode *link)
{	
	short int *array;
   ...
   * of those hexadecimal will occupy only one space on this string*/
	array = (short int *)calloc(SIZE, sizeof(short int));
	if(array == NULL)
	{
		perror("Calloc");
		exit(EXIT_FAILURE);
	...
	qsort(array, (index), sizeof(short int), alpha_cmp);
	array[index++] = FALSE; /* NULL terminator */
	return array;
}
 ...
 * */
char *dfs(DataNode *tree, short int c)
{
	char *bin_path;
	int index;
	short int found;
 ...
 * */
short int dfs_traversal(DataNode *tree,\
								char *r_str, short int target, int index)
{
 ...
 * 	None
 * */
void free_string(short int *str)
{
	free(str);
}
